...
However, as being notified on every login/logout or group membership change would be unnecessary and maybe even bothersome, the system should instead notify on action with possibly malicious intent,
eg. \textbf{repeating login attempts} or \textbf{enrolling of a user into a highly priviledged group} (admins, sales, etc.).
...
In case the reciever is written in another lower-level binding, the reciever will have to be editted accordingly. \\
